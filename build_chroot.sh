...
meson setup --prefix=/tmp/gst-plugins-good/usr -Dauto_features=disabled -Dgstreamer:tools=enabled -Dgst-full-plugins=coreelements -Dgood=enabled -Dbad=enabled -Dgst-plugins -Dgst-plugins-good:qt5=enabled build 
cd build
ninja
ninja install
